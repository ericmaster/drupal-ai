...
- Similar to how `ddev composer update --lock` works for dependencies
...
ddev drush upgrade_status:analyze module_name
...
- Generated by `ddev composer patches-relock`
...
3. Run `ddev composer install` or `ddev composer update drupal/audiofield --with-all-dependencies`
...
ddev composer require drupal/module_name:1.x-dev --with-all-dependencies
```
...
```bash
ddev composer update drupal/module_name --with-all-dependencies
...
ddev exec vendor/bin/phpunit docroot/modules/contrib/module_name/tests/
```
...
ddev composer show drupal/module_name | grep versions
...
# ddev composer install
# Add second patch, ddev composer install
...
4. Edit UserLimit.php directly after ensuring ddev composer patches are applied
...
```bash
# Update module
ddev composer require drupal/module_name:^2.0 --with-all-dependencies
