...
# Compare with local
echo "=== LOCAL ===" && grep "setting" config/sync/config.name.yml
echo "=== REMOTE ===" && grep "setting" /tmp/remote_config.yml

# Or use diff
diff -u config/sync/config.name.yml /tmp/remote_config.yml
```
...
```bash
# Read local config
cat config/sync/config.name.yml

# Edit the file (use Edit tool)
# Then verify changes
git diff config/sync/config.name.yml

# Commit changes
git add config/sync/config.name.yml
...
echo "=== LOCAL: block.block.custom_hero ===" && \
  grep -A2 "cta_text:" config/sync/block.block.custom_hero.yml

echo "=== LOCAL: mymodule.settings ===" && \
  grep "cta_text\|cta_url" config/sync/mymodule.settings.yml

# 3. Manually edit files using Edit tool to match dev values

# 4. Review and commit changes
git diff config/sync/block.block.custom_hero.yml \
  config/sync/mymodule.settings.yml

git add config/sync/*.yml
...
cp /tmp/server.yml config/sync/search_api.server.solr_server.yml
cp /tmp/index.yml config/sync/search_api.index.content.yml

# 3. If server was renamed, remove old config
git rm config/sync/search_api.server.old_name.yml

# 4. Review and commit
git diff config/sync/search_api.*
git add config/sync/search_api.*
...
6. **Verify after restore**: If files get deleted from working directory, restore from git:
   ```bash
   git checkout HEAD -- config/sync/file.yml
   ```
...
git checkout HEAD -- config/sync/search_api.*.yml

# Verify files exist
ls -la config/sync/search_api.*.yml
```
