...
> **Note:** The test structure below assumes your project uses [DTT (Drupal Test Traits)](https://gitlab.com/weitzman/drupal-test-traits) with ExistingSite tests located at the project root `tests/src/ExistingSite/`. If your project uses a different testing framework (e.g., PHPUnit Functional, Kernel, Unit), adjust accordingly.

Add a test stub in `tests/src/ExistingSite/` (at the project root, not inside the module):

**tests/src/ExistingSite/MyModuleTest.php**
```php
<?php
 ...
 * @group my_module
 */
final class MyModuleTest extends ExistingSiteBase {

  /**
   * Tests that the module is installed.
   */
  public function testModuleInstalled(): void {
    $this->assertTrue(
      \Drupal::moduleHandler()->moduleExists('my_module'),
      'my_module module is installed.'
    );
  }
...
│   └── Service/
│       └── ExampleService.php
# Tests live at project root tests/src/ExistingSite/, not inside the module
tests/src/ExistingSite/
└── MyModuleExampleTest.php
```
