...
> **Path note:** Commands below use `docroot/` as the web root. Adjust to match your project layout (common alternatives: `web/`, `html/`).
...
ls -la docroot/sites/default/files/
...
grep -r "db_query.*\$" docroot/modules/custom/
grep -r '->query.*".*\$' docroot/modules/custom/

# Find potential XSS (raw output without sanitization)
grep -r "#markup.*\$" docroot/modules/custom/
grep -r "Markup::create.*\$" docroot/modules/custom/

# Find hardcoded credentials
grep -ri "password\s*=" docroot/modules/custom/
grep -ri "api_key\s*=" docroot/modules/custom/
grep -ri "secret" docroot/modules/custom/

# Find static service calls (testability/security smell)
grep -r "\\Drupal::" docroot/modules/custom/src/
```
