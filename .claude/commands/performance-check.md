...
> **Path note:** Commands below use `docroot/` as the web root. Adjust to match your project layout (common alternatives: `web/`, `html/`). Similarly, `docroot/modules/custom/` assumes the standard custom modules location — update if yours differs.
...
grep -r "getQuery" docroot/modules/custom/
grep -r "entityQuery" docroot/modules/custom/
```
...
grep -r "->load(" docroot/modules/custom/ | head -20
grep -r "entity_load" docroot/modules/custom/ | head -20

# Find queries inside foreach
grep -rB5 "->load(" docroot/modules/custom/ | grep -A5 "foreach"
```
...
grep -r "public function get" docroot/modules/custom/src/Service/ | head -20
```
...
find docroot/modules/custom -name "*.libraries.yml" -exec cat {} \;

# Look for inline JS/CSS (should use libraries)
grep -r "<script" docroot/modules/custom/templates/
grep -r "<style" docroot/modules/custom/templates/
```
